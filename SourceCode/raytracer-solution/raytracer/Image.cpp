...
ImageDiff Image::diff(const Image& other) const
{
	ImageDiff imgDiff{ width, height };
	auto& diffs = imgDiff.diffs;
	...
	for (size_t i = 0; i < getWidth() * getHeight(); ++i) {
		if (data[i] - other.data[i] != Color{ 0, 0, 0 }) {
			diffs.emplace_back(linearToXY(i));
		}
	}
...
void Image::writeToFile(std::filesystem::path filename) const {
	...
	if (ext == ".png") {
		...
		int result = stbi_write_png(filename.string().c_str(), intWidth, intHeight, channels, pngData.data(), stride_bytes);
		if (result == 0) {
			throw std::runtime_error("Failed to write PNG file");
		}
	}
	else if (ext == ".bmp") {
		int result = stbi_write_bmp(filename.string().c_str(), intWidth, intHeight, channels, pngData.data());
		if (result == 0) {
			throw std::runtime_error("Failed to write BMP file");
		}
	}
	else { throw std::runtime_error("unexpected extension"); }
...
void ImageDiff::writeToFile(std::filesystem::path filename) const
{
	Color diffMarker = { 255, 0, 0 };
	Image image{ width, height };
	for (const auto& diff : diffs) {
		image(diff.x, diff.y) = diffMarker;
	}
